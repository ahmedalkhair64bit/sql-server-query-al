...
Full pipeline, measured on 2026-09-25 with DeepSeek V4-Pro as the analyst (maximum response length 32,000) and `jev-latest`: 14 of 18 plans correct, with the analyst proposing a correct option on every plan. That includes the complex batch in `fixtures/eval/complex-report.sqlplan`. Results vary between runs. The misses were Jev preferring a new index over OPTION (RECOMPILE) for a filtered index, an equally scored rewrite over an index for an index spool, and two cases where Jev's low confidence in how well an option fits the bottleneck blocked the best option. Reasoning models like DeepSeek take 2 to 5 minutes per analysis.
