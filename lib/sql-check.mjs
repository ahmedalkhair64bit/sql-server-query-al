...
export function parseDdl(sql) {
  ...
  return { text, error, indexes, statistics };
}
...
export function checkCandidateSql(c, d) {
  ...
  if (c.option_type === "index") {
    if (!ddl.indexes.length)
  ...
  if (c.option_type === "rewrite") {
    ...
    const SETUP =
      /^\s*(?:;\s*)?(?:CREATE\s+TABLE\s+#|DECLARE\b|SET\s+(?:NOCOUNT|@)|INSERT\s+(?:INTO\s+)?#|SELECT\b[^;]*?\bINTO\s+#|DROP\s+TABLE\s+(?:IF\s+EXISTS\s+)?#)[^;]*;/i;
    let rest = ddl.text,
      setup = false;
    ...
    if (!rewrite)
      ...
      warnings.push(
        "The rewrite adds setup statements (temp tables or variables) before the query; run them in the same batch.",
      );
  }
