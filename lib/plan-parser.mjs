...
const ONLY = "SQL Server Query AI analyses SQL Server ShowPlan XML only.";
// Plans from other engines, or SQL Server's text formats, get a message that names what was uploaded.
const FOREIGN = [
  [
    /"Node Type"\s*:|"Plan"\s*:\s*\{/,
    `This is a PostgreSQL EXPLAIN (FORMAT JSON) plan. ${ONLY}`,
  ],
  [
    /<explain[^>]*postgresql\.org/i,
    `This is a PostgreSQL EXPLAIN (FORMAT XML) plan. ${ONLY}`,
  ],
  [
    /\(cost=\d+\.\d+\.\.\d+\.\d+ rows=\d+/,
    `This is a PostgreSQL EXPLAIN text plan. ${ONLY}`,
  ],
  [/"query_block"\s*:/, `This is a MySQL EXPLAIN FORMAT=JSON plan. ${ONLY}`],
  [
    /->\s.+\(cost=[\d.]+ rows=[\d.]+\)|\(actual time=[\d.]+\.\.[\d.]+ rows=/,
    `This is a MySQL EXPLAIN (tree or ANALYZE) plan. ${ONLY}`,
  ],
  [
    /\bselect_type\b[\s\S]*\bpossible_keys\b/,
    `This is a MySQL tabular EXPLAIN. ${ONLY}`,
  ],
  [
    /Plan hash value:|PLAN_TABLE_OUTPUT|\|\s*Id\s*\|\s*Operation\s*\|/,
    `This is an Oracle execution plan (DBMS_XPLAN). ${ONLY}`,
  ],
  [
    /^\s*\|--|\n\s*\|--/,
    "This is a SQL Server text plan (SET SHOWPLAN_TEXT or STATISTICS PROFILE). Save the plan as XML instead: in SSMS, right-click the graphical plan and choose Save Execution Plan As, or use SET STATISTICS XML ON.",
  ],
  [
    /^\s*(SELECT|WITH|INSERT|UPDATE|DELETE|MERGE|EXEC)\b/i,
    "This is SQL text, not an execution plan. Run the query with Include Actual Execution Plan (Ctrl+M) in SSMS and save the plan as a .sqlplan file.",
  ],
];
/** Explains what a non-ShowPlan upload is, from its first characters; null when it is not recognised. */
export function identifyForeignPlan(head) {
  const text = String(head ?? "").slice(0, 8000);
  for (const [re, message] of FOREIGN) if (re.test(text)) return message;
  return null;
}
const n = (v) =>
...
export function createPlanParser(onStatement) {
  ...
  parser.on("error", (e) => {
    throw new PlanError(
      identifyForeignPlan(head) ??
        `The XML stopped parsing: ${e.message.slice(0, 180)}`,
    );
  });
  parser.on("opentag", (tag) => {
    ...
    if (name === "RelOp") {
      ...
      const o = {
        ...
        seekColumns: [],
        predicate: null,
        // For the rule-built fix options (lib/rule-options.mjs): what the operator returns, a sort's order,
        // and the relatives a fix needs, recorded while streaming because only the top operators are kept.
        outputColumns: [],
        orderBy: [],
        parentId: null,
        sourceObject: null,
      ...
      const parentEntry = ops.at(-1)?.d === d ? ops.at(-1) : null;
      if (parentEntry) {
        o.parentId = parentEntry.o.id;
        // TOP (1) ... ORDER BY above a spool or seek: the order a supporting index should have.
        const sorted = [parentEntry, ops.at(-2)].find(
          (e) => e && e.d === d && e.o.orderBy.length,
        );
        if (sorted) o.orderAbove = sorted.o.orderBy.slice(0, 4);
        // A key lookup's partner: the index seek on the nested loop's outer side, already closed.
        if (parentEntry.firstChild) o.siblingBefore = parentEntry.firstChild;
      }
      ops.push({
        depth,
        d,
        ...
        threadRows: [],
        threads: 0,
        // The statement's top operator: if it never reached end of scan, the execution did not finish.
        isRoot: stack.at(-2)?.name === "QueryPlan",
        endOfScans: null,
      });
      d.operatorCount++;
    ...
    if (name === "RunTimeCountersPerThread" && o) {
      ...
      if (n(a.Thread) > 0 && (n(a.ActualExecutions ?? a.ActualExecs) ?? 0) > 0)
        active.threads++;
      if (n(a.ActualEndOfScans) !== null)
        active.endOfScans = (active.endOfScans ?? 0) + n(a.ActualEndOfScans);
    ...
    if (
      ...
      // Internal worktables (a cursor's tempdb CWT_PrimaryKey) name only a database and an index.
      o.object = fullName(a) + (a.Index ? `.${unbrack(a.Index)}` : "");
      if (a.Table && a.Index && d.indexes.length < 50) {
        const used = compact({
          table: fullName(a),
    ...
    if (
      o &&
      name === "ColumnReference" &&
      stack.at(-2)?.name === "OutputList" &&
      stack.at(-3)?.name === "RelOp" &&
      a.Column &&
      o.outputColumns.length < 16
    )
      o.outputColumns.push({
        table: fullName(a) || null,
        column: unbrack(a.Column),
      });
    if (o && name === "OrderByColumn")
      active.orderDesc = a.Ascending === "false" || a.Ascending === "0";
    if (
      o &&
      name === "ColumnReference" &&
      stack.at(-2)?.name === "OrderByColumn" &&
      a.Column &&
      o.orderBy.length < 8
    )
      o.orderBy.push({
        table: fullName(a) || null,
        column: unbrack(a.Column),
        desc: !!active.orderDesc,
      });
    // A spool stores rows from the table below it: remember which, for an index that replaces it.
    if (name === "Object" && a.Table)
      for (let i = ops.length - 1; i >= 0 && ops[i].d === d; i--)
        if (/Spool/.test(ops[i].o.op) && !ops[i].o.sourceObject)
          ops[i].o.sourceObject = fullName(a);
    if (
      o &&
      name === "ColumnReference" &&
    ...
    // Some warnings are attributes of <Warnings> itself: <Warnings NoJoinPredicate="1"/> on a join,
    // <Warnings UnmatchedIndexes="true"/> on the plan. Reading only child elements missed them on real plans.
    if (name === "Warnings")
      for (const [k, v] of Object.entries(a))
        if (v === "1" || v === "true")
          addWarning(
            d,
            o && stack.at(-2)?.name === "RelOp"
              ? `Node ${o.id} ${o.op}: ${k}`
              : k,
          );
    // The filtered indexes a parameterized plan could not use: <QueryPlan><UnmatchedIndexes><Parameterization>
    // <Object .../>. Name them, so the finding can say which index.
    if (
      name === "Object" &&
      stack.some((s) => s.name === "UnmatchedIndexes") &&
      !stack.some((s) => s.name === "Warnings")
    )
      addWarning(
        d,
        `UnmatchedIndexes index=${fullName(a)}${a.Index ? `.${unbrack(a.Index)}` : ""}`,
      );
  ...
  parser.on("closetag", () => {
    ...
    if (ops.at(-1)?.depth === depth) {
      ...
      if (
        ...
        entry.endOfScans === 0 &&
        (o.execs ?? 0) > 0 &&
        !owner.incompleteExecution
      ) {
        owner.incompleteExecution = true;
        owner.coverage.push(
          "The statement did not finish (cancelled, timed out or failed): row counts and times are partial.",
        );
      }
      ...
      if (parent && parent.d === owner && !parent.firstChild)
        parent.firstChild = {
          id: o.id,
          object: o.object,
          seekColumns: o.seekColumns.slice(0, 8),
        };
      ...
      if (o.actualRows !== null && o.execs !== 0) {
        // A parallel operator reports one execution per thread; that is one execution of the operator, not
        // eight. Inner sides of nested loops run far more often than there are threads and keep their count.
        const logical =
          entry.threads > 1 && (o.execs ?? 1) <= entry.threads
            ? 1
            : (o.execs ?? 1);
        const perExec = o.actualRows / Math.max(logical, 1);
  ...
  return {
    write(text) {
      ...
      if (head.length < 8000) head += text.slice(0, 8000 - head.length);
      parser.write(text);
    },
    end() {
      parser.close();
  ...
  return [
    {
      ...
      value: compact({
        ...
        incompleteExecution: d.incompleteExecution,
...
/**
 * The statement to analyse first in a batch: the one that ran longest when the plan measured time,
 * otherwise the costliest estimate. Shared by the upload worker and the evaluation tools.
 */
export const statementWeight = (d) =>
  d.queryTime?.elapsedMs != null
    ? 1e12 + d.queryTime.elapsedMs
    : (d.subtreeCost ?? 0);
export const recommendStatement = (digests) =>
  digests.reduce(
    (best, d) => (statementWeight(d) > statementWeight(best) ? d : best),
    digests[0],
  );
