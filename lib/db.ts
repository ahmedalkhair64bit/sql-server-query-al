...
function openDb(): DatabaseSync {
  ...
  const cols = new Set(
    (d.prepare("PRAGMA table_info(analyses)").all() as { name: string }[]).map(
      (c) => c.name,
    ),
  );
  ...
  if (!settingCols.has("analyst_status"))
...
const one = <T>(sql: string, ...p: Row[]): T | null =>
  plain<T>(db.prepare(sql).get(...p));
...
export function createUser(email: string, passHash: string): string {
  ...
  ).run(id, email, passHash, now());
  return id;
}
export const userByEmail = (email: string) =>
  one<{ id: string; pass: string }>(
    "SELECT id, pass FROM users WHERE email = ?",
    email,
  );
export const userCount = () =>
  one<{ n: number }>("SELECT COUNT(*) AS n FROM users")?.n ?? 0;
export const userEmail = (id: string) =>
  one<{ email: string }>("SELECT email FROM users WHERE id = ?", id);
...
export const dropOtherSessions = (userId: string, keepHash: string) =>
  db
    ...
    .run(userId, keepHash);
export const userPassword = (id: string) =>
  one<{ pass: string }>("SELECT pass FROM users WHERE id = ?", id);
...
export function setModelStatus(
  ...
  db.prepare(
    `UPDATE settings SET ${target === "analyst" ? "analyst_status" : "jev_status"} = ? WHERE user_id = ?`,
  ).run(status, userId);
}
export const setSendSql = (userId: string, on: boolean) =>
  db
    .prepare("UPDATE settings SET send_sql = ? WHERE user_id = ?")
    .run(on ? 1 : 0, userId);
export const analysisIdsFor = (userId: string) =>
  (
    db.prepare("SELECT id FROM analyses WHERE user_id = ?").all(userId) as {
      id: string;
    }[]
  ).map((r) => r.id);
export const exportAnalyses = (userId: string) =>
  (
    db
      .prepare(
        `SELECT id, title, status, error, created_at, digest, candidates, verdict, comparison
...
export function saveSettings(userId: string, s: SettingsInput) {
  ...
  db.prepare(
    `INSERT INTO settings (user_id, analyst_base_url, analyst_key, analyst_model, analyst_extra,
...
export function patchAnalysis(id: string, p: Record<string, string>) {
  ...
  db.prepare(
    `UPDATE analyses SET ${keys.map((k) => `${k} = ?`).join(", ")}, updated_at = ? WHERE id = ?`,
...
export const listAnalyses = (userId: string) =>
  (
    db
      .prepare(
        "SELECT id, title, status, created_at FROM analyses WHERE user_id = ? ORDER BY created_at DESC",
      )
      .all(userId) as {
      id: string;
      title: string;
...
export const renameAnalysis = (id: string, userId: string, title: string) =>
  db
    .prepare(
      "UPDATE analyses SET title = ?, updated_at = ? WHERE id = ? AND user_id = ?",
    )
