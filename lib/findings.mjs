...
export function detectFindings(d) {
  ...
  if (warnings.some((w) => /UnmatchedIndexes/.test(w))) {
    const names = warnings
      .map((w) => /^UnmatchedIndexes index=(.+)$/.exec(w)?.[1])
      .filter(Boolean);
    add(
      "unmatched_indexes",
      "warning",
      names.length
        ? `Filtered index ${names.join(", ")} could not be used`
  ...
  // At most three findings per rule: a plan with nine index spools produced nine identical findings, which
  // crowded the rest of the evidence out of the models' context. The third says how many more there are.
  const MAX_PER_RULE = 3;
  const seen = new Map();
  const kept = [];
  for (const f of out.sort((a, b) => rank[a.severity] - rank[b.severity])) {
    const n = (seen.get(f.rule) ?? 0) + 1;
    seen.set(f.rule, n);
    if (n <= MAX_PER_RULE) kept.push(f);
  }
  for (const [rule, n] of seen)
    if (n > MAX_PER_RULE) {
      const last = kept.filter((f) => f.rule === rule).at(-1);
      last.detail += ` (${n - MAX_PER_RULE} more like this in the plan.)`;
    }
  return kept;
}
