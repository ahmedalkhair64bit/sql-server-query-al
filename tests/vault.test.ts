...
const { saveSettingsForm, analystConfig, settingsView, jevKey } = await import("../lib/settings.ts");
...
test("blank secret keeps the stored key", () => {
  ...
  saveSettingsForm(u, fd({ analyst_base_url: "https://api/x/v1/", analyst_key: "sk-1111", analyst_model: "gpt",
    analyst_extra: '{"temperature":0.2}', jev_key: "ts-1", jev_model: "jev-latest" }));
  assert.equal(analystConfig(u)?.apiKey, "sk-1111");

  saveSettingsForm(u, fd({ analyst_base_url: "https://api/y/v1", analyst_key: "", analyst_model: "gpt",
  ...
  assert.deepEqual(c.extra, {}, "bad JSON falls back to empty params");
